            ...
            <el-col :span="8">
              <el-form-item label="套餐类型">
                <el-select :clearable="true" v-model="formAllInfo.cardType" filterable placeholder="请选择">
                  <el-option :label="item.typename" :value="item.typeCode" v-for="item in cardtype" :key="item.id"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
    ...
    methods:{
        ...
        async wxCardTypeAndClassFn(){
            ...
            try{
                ...
                "/fr/MyWallet/portCardTypeAndClass",
        ...
        async reChargeSearchFn(){
          this.$refs["ruleForm"].validate(async (valid) => {
            if (valid) {
              ...
              try {
                  const data = await this.$axios.post(
                  "/fr/MyWallet/reChargeCardSearch",
                  this._qs.stringify(paramsObj),
                  );
                  if(data.errcode==0){
